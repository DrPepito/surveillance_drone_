...
# on essaie d'importer le bridge HIL UDP, si ça marche pas on met juste des None
try:
...
# quelques utilitaires qu'on réutilise partout dans le code
def _clamp(v, a, b):
...
# interpolation de couleur entre deux QColor selon un facteur t entre 0 et 1
...
# EMA classique : lisse une valeur en la rapprochant progressivement de la cible
def _ema(prev, cur, alpha=0.12):
    return prev + alpha * (cur - prev)

# même chose mais pour les angles, gère le wrap autour de ±π pour éviter les sauts
...
# cette classe garde une version lissée de tous les paramètres du drone
# l'idée c'est d'éviter les affichages qui sautent partout sur le HUD
class Smoothed:
    A_FAST  = 0.22
    A_MED   = 0.20
    A_ANG   = 0.18
    ...
    def __init__(self):
        ...
        self.fpm_y    = 0.0
        self.fpm_vis  = 0.0
        self.att_t    = 0.0  # facteur 0→1 qui indique à quel point l'attitude est critique
        self.alt_t    = 0.0  # même chose pour l'altitude
    ...
    def update(self, s: DroneState):
        ...
        # on lisse toutes les valeurs physiques du drone
        ...
        # att_t monte vers 1 quand le drone dépasse 8° d'inclinaison, maxe à 28°
        ...
        # alt_t monte vers 1 quand on dépasse 8m, maxe à 16m
        target_alt_t = _clamp((s.position.z - 8.0) / 8.0, 0.0, 1.0)
        self.alt_t = _ema(self.alt_t, target_alt_t, ac)

        # calcul du Flight Path Marker en coordonnées écran
        # on projette la vitesse dans le repère "vue caméra" du drone
        ...
        if v_horiz > 0.05 or abs(vz) > 0.05:
            ...
            # le drone est quasi immobile, le FPM revient doucement au centre
...
class HUDWidget(QWidget):
    ...
    # couleurs fixes, les mêmes en mode jour et nuit
    ...
    TRAIL_MAX = 150   # nombre max de points gardés pour la traîne radar
    BOT_H     = 80    # hauteur de la barre du bas (moteurs + batterie)
    ...
    def __init__(self, state: DroneState):
        ...
        # état de la détection personne, mis à jour depuis le thread vision
        self._personne_detectee  = False
        self._personne_confiance = 0.0
 
        self._trail        = []   # historique de positions pour la traîne radar
        self._keyboard_ref = None
        self._hil_ref      = None
        self._mode_ing     = False   # overlay ingénieur HIL activé ou non
        self.setMinimumSize(800, 700)

        # variables d'état internes v3
        self._day_mode       = False
        self._blink_tick     = 0       # compteur pour les effets de clignotement
        self._peak_speed     = 0.0     # vitesse max atteinte depuis le dernier reset
        self._bat_prev_pct   = None
        self._bat_drain_rate = 0.0     # consommation batterie estimée en %/s
        self._bat_time_rem   = None    # temps de vol restant estimé en secondes

        # snapshot de la dernière trame UDP envoyée, pour l'overlay ingénieur
    ...
    # retourne True sur la moitié haute de la période, pour faire clignoter des trucs
    def _blink_on(self, period_ticks=20) -> bool:
        return (self._blink_tick % period_ticks) < (period_ticks // 2)

    # détecte si le drone est en stationnaire : vitesse quasi nulle dans les 3 axes
    ...
    def tick_smooth(self):
        # on met à jour les valeurs lissées et on fait avancer les variables internes
        self.smooth.update(self.state)
        s  = self.state
        sm = self.smooth

        # on enregistre la position uniquement quand les moteurs tournent
        if s.moteurs_armes:
        ...
        # estimation du temps de vol restant basée sur la consommation récente
        if self._bat_prev_pct is not None:
            ...
            if self._bat_drain_rate > 1e-4:
                reserve = 10.0  # on garde 10% de réserve dans le calcul
    ...
    # couleur de l'indicateur d'attitude : vert → jaune → rouge selon l'inclinaison
    ...
    # couleur de la jauge altitude : cyan en bas, orange quand on monte haut
    def _alt_gauge_color(self) -> QColor:
        return _lerp_color(self.C_CYAN, self.C_ORANGE, self.smooth.alt_t)

    # couleur batterie : vert au-dessus de 45%, orange entre 20% et 45%, rouge en dessous
    ...
    # couleur d'un moteur : cyan au repos, jaune en milieu de course, rouge à fond
    ...
    def paintEvent(self, event):
        ...
        # on calcule les zones de chaque section du HUD
        HDR_H   = 44
        BOT_H   = self.BOT_H
        LEFT_W  = 156
        RIGHT_W = 86
        ...
        HOR_H   = int(BODY_H * 0.60)   # l'horizon prend 60% de la zone centrale
        RAD_H   = BODY_H - HOR_H       # le radar prend le reste
    ...
    def _draw_header(self, p, x, y, w, h):
        ...
        # chaque mode de vol a sa propre couleur de badge
        MODE_COL = {
            ...
            DroneState.MODE_HOME:      QColor("#2060A0"),
        }
        ...
        # badge HOLD qui s'affiche quand le drone est en stationnaire
        ...
        # petit rond coloré qui indique l'état d'attitude
        ac = self._att_color()
        ...
        # indicateur HIL en haut si le bridge est actif
        hil_txt = ""
        ...
        # vitesse max enregistrée depuis le dernier reset
        ...
        # tension et pourcentage batterie, avec statut HIL si connecté
        p.setPen(QPen(bc))
        p.setFont(QFont("Monospace", 9, QFont.Weight.Bold))
        ...
        # temps de vol restant estimé, affiché uniquement quand les moteurs tournent
        ...
        # petit label coin bas gauche qui indique le mode actif jour/nuit
    ...
    def _draw_left(self, p, x, y, w, h):
        ...
        # la vitesse verticale passe au orange quand elle dépasse 3 m/s
        vz_col = _lerp_color(self.C_CYAN, self.C_ORANGE,
                             _clamp(abs(sm.vz) / 3.0, 0, 1))

        # liste des métriques à afficher dans le panneau gauche
        metrics = [
        ...
        # si le clavier a des consignes de vitesse actives on les affiche aussi
        ...
        # altitude cible en bas du panneau, séparée visuellement
        ya = y + 20 + n * bloc_h + 8
        p.setPen(QPen(self.C_MUTED)); p.setFont(QFont("Monospace", 7))
    ...
    def _draw_key_hints(self, p, x, y, w, h):
        # rappel des raccourcis clavier en bas du panneau gauche
    ...
    def _draw_horizon(self, p, x, y, w, h):
        ...
        # décalage vertical de l'horizon selon le pitch
        ...
        # couche 1 : vidéo brute en fond, remplace le fond bleu si la caméra est active
        p.save()
        ...
        if qimg is not None:
            # on redimensionne l'image pour couvrir toute la zone sans déformer
            scaled = qimg.scaled(
                horiz_w - 4, h - 4,
            ...
            # pas de signal caméra, on affiche un placeholder
        ...
        # couche 2 : horizon synthétique semi-transparent par-dessus la vidéo
        # l'opacité à 0.60 permet de voir la vidéo en dessous
        p.save()
        p.setClipRect(x + 2, y + 2, horiz_w - 4, h - 4)
        ...
        # graduations de tangage de -30° à +30° tous les 5°
        p.setFont(QFont("Monospace", 7))
        for deg in range(-30, 31, 5):
            if deg == 0:
                continue
            py_  = int(-deg * 3.5)
            ll   = 52 if deg % 10 == 0 else 26   # les traits de 10° sont plus longs
            alp  = 220 if deg % 10 == 0 else 130
            p.setOpacity(alp / 255 * 0.85)
            p.setPen(QPen(QColor(0, 0, 0, alp), 0.9))
        ...
        # couche 3 : scanlines légères pour donner un effet écran HUD
        ...
        # couche 4 : instruments HUD à opacité pleine par-dessus tout le reste
        p.setOpacity(1.0)
        cx_h = x + horiz_w // 2
 
        # Flight Path Marker : indique la direction réelle du déplacement
        ...
        # chevron central qui représente la référence d'assiette du drone
        ...
        # arc de roulis en haut de l'horizon
        self._draw_roll_arc(p, cx_h, y + 22, 58, sm.roll, chevron_c)
 
        # ligne de glide path visible uniquement en phase d'atterrissage ou décollage
        ...
        # bordure colorée de la zone horizon qui vire au rouge si attitude critique
        ...
        # ruban de cap en bas de l'horizon
        self._draw_heading_tape(p, x, y, horiz_w, h, sm.yaw)
 
        # étiquettes vitesse sol et altitude superposées sur l'horizon
        p.setPen(QPen(self.C_MUTED))
        p.setFont(QFont("Monospace", 7))
        ...
        # badge CAM LIVE en bas à droite si la caméra tourne
        ...
        # couche 5 : warning personne détectée, s'affiche par-dessus tout si actif
        if self._personne_detectee:
            WARN_W = 280
            WARN_H = 32
            wx = cx_h - WARN_W // 2
            wy = y + h - 52   # juste au-dessus du ruban de cap
        ...
        # vario à droite de l'horizon, en dehors du clip principal
        self._draw_vario(p, x + horiz_w + VARIO_MARG // 2, y, VARIO_W, h, sm.vz)
 

    def _draw_vario(self, p, x, y, w, h, vz: float):
        # variomètre vertical : vert vers le haut, orange/rouge vers le bas
        MARG_V = 20
        bar_h  = h - MARG_V * 2
        bar_x  = x + (w - 16) // 2
        bar_w  = 16
        ...
        # triangle indicateur qui pointe sur la valeur actuelle
    ...
    def _draw_heading_tape(self, p, x, y, w, h, yaw):
        # ruban de cap en bas de l'horizon, avec les cardinaux N/E/S/W mis en avant
        TAPE_H  = 28
        TAPE_Y  = y + h - TAPE_H - 2
        cx      = x + w // 2
        ...
        p.setClipping(False)
        # triangle jaune centré qui indique le cap courant
    ...
    def _draw_roll_arc(self, p, cx, cy, r, roll, indicator_color):
        # arc de roulis avec graduations et triangle mobile qui suit le roulis actuel
    ...
    def _draw_radar(self, p, x, y, w, h):
        ...
        cx = x + w//2; cy = y + h//2
        rayon = min(w, h)//2 - 14
        echelle = rayon / 30.0   # 30m = rayon complet du radar

        # anneaux de distance, le dernier devient rouge si la zone de sécurité est dépassée
        ...
        # zone rouge hachurée si le drone sort de la zone de sécurité
        ...
        # traîne de la trajectoire : les points récents sont plus épais et plus lumineux
        ...
        # ligne pointillée entre le centre et la position du drone
        p.setPen(QPen(QColor(91, 207, 234, 35), 0.5, Qt.PenStyle.DotLine))
        p.drawLine(cx, cy, cx+dx, cy+dy)

        # vecteur vitesse depuis la position du drone
        vscale = echelle * 1.0
        ...
        # indicateur de cap depuis la position du drone
        yaw_len = 18
    ...
    def _drone_triangle(self, p, x, y, yaw):
        # curseur du drone sur le radar : triangle violet avec halo et point blanc au centre
        t  = 14
        ...
        # deux halos concentriques pour l'effet de présence
    ...
    def _draw_gauges(self, p, x, y, w, h):
        # panneau droit : jauge altitude et jauge throttle côte à côte
        sm = self.smooth
        p.fillRect(x, y, w, h, self.C_PANEL)
        ...
        # ligne pointillée qui indique l'altitude cible
        ...
        # jauge throttle à côté si elle rentre dans l'espace disponible
        tx = x + 8 + JAU_W + 10
        if tx + JAU_W < x + w - 2:
            p.setPen(QPen(QColor(48, 54, 68, 200), 0.5))
            p.drawRect(tx, y+MARG_T, JAU_W, JAU_H)
            rem2 = int(JAU_H * sm.throttle)
            # couleur qui s'éloigne du cyan quand on s'écarte du hover
            ...
            # ligne qui marque le niveau de throttle hover
    ...
    def _draw_bottom(self, p, x, y, w, h):
        # barre inférieure : barres des 4 moteurs à gauche, batterie à droite
        sm = self.smooth
        s  = self.state
        ...
        # une barre par moteur, couleur selon l'intensité
        for i, m in enumerate(sm.moteurs):
            ...
            # petite ligne à mi-hauteur qui marque le niveau hover
        ...
        # séparateur vertical entre moteurs et batterie
        ...
        # batterie critique : on fait clignoter toute la section
        bat_visible = True
        if pct < BAT_CRIT_PCT:
        ...
        # message de statut batterie selon le niveau
        if pct > 50:
    ...
    def _draw_alerts(self, p, x, y, w, h):
        # alertes superposées au centre du HUD, clignotantes, empilées verticalement
        sm = self.smooth
        s  = self.state
        alerts = []
    ...
    def _draw_hil_overlay(self, p, w, h):
        # overlay ingénieur : affiché quand on appuie sur I
        # montre l'état du bridge HIL et la dernière trame UDP brute
        s   = self.state
        OW  = 420; OH = h - 20
        OX  = w - OW - 10; OY = 10
        ...
        # les champs sont affichés en deux colonnes pour gagner de la place
        sections = [
        ...
        # en-tête de la zone JSON avec adresse cible et compteur de trames
        ...
        # zone de texte JSON brut, on coupe les lignes pour qu'elles rentrent
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._last_tick_time = time.monotonic()   # point de départ pour le DT dynamique
        ...
        # bridge série vers l'ESP32 sur COM7
        self.hil_serial = HilBridgeSerial(port="COM7")
        self.hil_serial.demarrer()

        # bridge HIL UDP vers un simulateur externe si disponible
        if HIL_DISPONIBLE:
            cfg = HilConfig(
                udp_actif    = True,
                udp_host     = "127.0.0.1",
        ...
        # layout principal : HUD à gauche, widgets orientation/graphs à droite
        conteneur = QWidget()
        ...
        # timer Qt qui déclenche le tick à 50 Hz
        self.timer = QTimer()
    ...
    def closeEvent(self, event):
        # on arrête proprement tous les threads avant de fermer
        if self.hil:
            self.hil.arreter()
        if self.hil_serial:
    ...
    def _tick(self):
        # on mesure le vrai temps écoulé depuis le dernier tick plutôt que d'utiliser 1/FPS fixe
        # ça rend la physique plus stable si le timer dérive un peu
        now = time.monotonic()
        dt  = now - self._last_tick_time
        dt  = max(0.005, min(dt, 0.05))   # on clamp entre 5ms et 50ms pour éviter les sauts
        self._last_tick_time = now

        s = self.state
        self.keyboard.update(s, dt)

        # on dépile les événements one-shot du clavier
        ...
        self._update_mode()
        self._appliquer_pids(dt)
        physics_update(s, dt)

        # on envoie la télémétrie sur UDP et série à chaque tick
    ...
    def _decollage(self):
        # on ne peut décoller que depuis le sol
    ...
    def _atterrissage(self):
        # on peut atterrir depuis vol, décollage ou retour home
    ...
    def _update_mode(self):
        ...
        # décollage terminé quand on atteint 95% de l'altitude cible
        ...
        # atterrissage terminé quand quasi au sol et vitesse verticale nulle
            ...
            # sécurité : si le drone touche le sol pendant HOME on coupe proprement
            if s.position.z <= 0.05 and abs(s.vitesse.z) < 0.1:
                s.mode_vol      = DroneState.MODE_SOL
                s.moteurs_armes = False
                s.moteurs       = [0.0, 0.0, 0.0, 0.0]
    ...
    def _appliquer_pids(self, dt: float):
        ...
        # PID altitude : calcule la correction throttle pour maintenir l'altitude cible
        ...
        # on convertit les consignes clavier du repère drone vers le repère monde
        ...
        # les PIDs de vitesse donnent des angles cibles en sortie
    ...
    def keyPressEvent(self, event: QKeyEvent):
        ...
        if key == Qt.Key.Key_I:
            ...
            # on change aussi la couleur de fond de la fenêtre selon le mode
...
if __name__ == "__main__":
    ...
    # thread de vision séparé pour ne pas bloquer la boucle Qt
    # il lit les frames de la webcam et envoie le résultat de détection au HUD
    def boucle_vision():
        from test import image_Comparateur
        while True:
            ...
            if frame is not None:
                ...
                # on passe juste le booléen et le score au HUD, pas l'image
