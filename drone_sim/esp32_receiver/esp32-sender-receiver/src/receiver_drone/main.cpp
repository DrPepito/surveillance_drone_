...
// ESP32-CAM envoie le stream MJPEG encapsulé en chunks sur UART
// Protocole simple : header 4 octets taille + données JPEG
#define CAM_UART_RX   16
#define CAM_UART_TX   17
#define CAM_BAUD      921600   // haut débit pour MJPEG
...
// Buffer frame JPEG courante (max 30Ko pour VGA JPEG)
...
// ── COMMANDES RC ──────────────────────────────────────────────────────────────
struct CmdRC {
  float roll     = 0.0f;   // -1.0 à +1.0
  float pitch    = 0.0f;
  float yaw      = 0.0f;
  float throttle = 0.0f;   //  0.0 à +1.0
  int   mode     = 0;
  bool  arm      = false;
  unsigned long ts = 0;
};
...
// On prépare la structure, l'envoi réel se fait dans sendToFC()
struct MSP_RC {
  uint16_t roll;     // 1000-2000 µs
  uint16_t pitch;
  uint16_t yaw;
  uint16_t throttle;
  uint16_t arm;      // 1000 = désarmé, 2000 = armé
};
...
// Convertit -1.0/+1.0 → 1000/2000 µs (format RC standard)
...
bool parseRC(const char* json) {
  ...
  // Convertir en microsecondes pour le FC
...
// Format MSP SET_RAW_RC : $M< + size + cmd + payload + checksum
void sendToFC() {
  if (!cmd.arm) return;   // sécurité : n'envoie rien si désarmé

  // Sécurité watchdog : si pas de commande depuis 500ms → tout à neutre
  ...
  // Payload : 8 canaux × 2 octets = 16 octets
  // Canaux : ROLL PITCH YAW THROTTLE AUX1(ARM) AUX2 AUX3 AUX4
  uint8_t  payload[16];
  uint16_t channels[8] = {
  ...
  uint8_t checksum = 16 ^ 200;  // size XOR cmd
...
// ── Relay RC vers ESP32 #1 (pour que le PC voie les commandes) ───────────────
...
// Protocole côté CAM (code esp32cam_uart_sender.ino) :
//   - Envoie 0xFF 0xD8 ... 0xFF 0xD9 (JPEG brut)
//   - On détecte SOI/EOI pour extraire les frames
void lireCAMUART() {
  ...
  while (Serial2.available()) {
    ...
    if (!inFrame) {
      ...
      // Détecte EOI JPEG : 0xFF 0xD9
      if (prevByte == 0xFF && b == 0xD9) {
        // Frame complète → copier dans frameBuf
...
void handleStream() {
  ...
  while (client.connected()) {
    ...
    // Copier la frame sous protection mutex
...
void setup() {
  ...
  Serial1.begin(FC_BAUD, SERIAL_8N1, FC_UART_RX, FC_UART_TX);   // FC
  Serial2.begin(CAM_BAUD, SERIAL_8N1, CAM_UART_RX, CAM_UART_TX); // CAM
  ...
  // WiFi client → se connecte à l'AP de ESP32 #1
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ip_sol = WiFi.gatewayIP();   // ESP32 #1 = gateway de l'AP
    sol_connecte = true;
